        ...
        <div class="card-header">
            <div class="row justify-content-between align-items-center flex-grow-1">
                    ...
                    <form action="{{ route('find_term_KH') }}" method="GET">
                        ...
                        <div class="input-group input-group-merge input-group-flush">
                            ...
                            <input id="datatableSearch" type="search" class="form-control"
                                ...
                                placeholder="Tìm kiếm tag khóa học" aria-label="Search users">
                ...
                <div class="col-sm-6 col-md-4 mb-3 mb-sm-0 d-flex justify-content-end">
                    <a href="{{ route('them_term_KH') }}" class="btn btn-success pt-1 pb-1 pr-2 pl-2">Thêm mới</a>
                </div>
            </div>
            <!-- End Row -->
        </div>
                                ...
                                <td>
                                    @if (session()->get('role') == 'admin' || session()->get('role') == 'nv')
                                        <a class="btn btn-sm btn-white"
                                            href="{{ route('page_edit_term_KH', ['id' => $ds_terms_khoa_hoc[$i]->id]) }}">
                                            <i class="fa-solid fa-pen-to-square" style="color: #1d158a;"></i>
                                        </a>
                                    @endif
                                    @if (session()->get('role') == 'admin')
                                        <a class="btn btn-sm btn-white"
                                            href="{{ route('delete_term_KH', ['id' => $ds_terms_khoa_hoc[$i]->id]) }}"
